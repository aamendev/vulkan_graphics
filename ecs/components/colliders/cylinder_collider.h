...
namespace Lina { namespace ECS { namespace Components {
    class CylinderCollider : public Collider
    {
        public:
            ...
            CylinderCollider(std::string tag): mHeight(10), mRadius(5)
            {
                mTag = tag;
                mRecentCollisions = {};
                mCallDefaults = true;
        ...
        CylinderCollider(std::string tag, f32 h, f32 r, Math::Point3D c): 
            mRadius(r), mHeight(h){
                mTag = tag;
                mRecentCollisions = {};
                mCallDefaults = true;
